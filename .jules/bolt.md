...
## 2025-03-22 - Array.forEach Overhead in High-Frequency Network Paths
**Learning:** Using `Array.prototype.forEach` inside hot paths like `sendBatch` (in `src/api/transportAdapter.js`) leads to the constant allocation of anonymous callback functions, generating significant Garbage Collection (GC) overhead during high-frequency network events.
**Action:** When iterating over arrays in performance-critical sections, rely on a standard `for` loop to avoid closure allocation and minimize GC pressure.
